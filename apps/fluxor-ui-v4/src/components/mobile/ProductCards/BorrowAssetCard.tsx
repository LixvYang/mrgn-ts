...
import { dynamicNumeralFormatter, percentFormatter, percentFormatterDyn, usdFormatter } from "@mrgnlabs/mrgn-common";
...
export const BorrowAssetCard = ({ asset, isConnected, onConnect }: BorrowAssetCardProps) => {
  ...
  const borrowedUsd = asset.bankCap.bankCapUsd;
  const utilization = asset.utilization.utilization;
    ...
    <Card className="bg-card/70 border border-border/60 shadow-sm transition-colors">
      <CardContent className="p-4">
        ...
        <div className="space-y-2 mb-4">
          ...
          <div className="flex justify-between items-center">
            <span className="text-xs text-muted-foreground">已借出</span>
            <div className="flex items-center gap-1.5">
              <span className="text-sm font-medium">{formatAmount(asset.bankCap.bankCap)}</span>
          ...
          {/* <div className="flex justify-between items-center">
            <span className="text-xs text-muted-foreground">可借额度</span>
            <div className="flex items-center gap-1.5">
              <span className="text-sm font-medium">{formatAmount(asset.deposits.available)}</span>
              <span className="text-xs text-muted-foreground bg-muted/50 px-2 py-0.5 rounded">
...
export default BorrowAssetCard;
