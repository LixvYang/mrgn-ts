...
import { LoginModal, MixinRegister } from "~/components/common/MixinWallet";
import {
  useRefreshUserData,
...
import { useSelectedAccount } from "@mrgnlabs/mrgn-state";
...
export const Navbar: FC = () => {
  ...
  const setSelectedAccountKey = useSetSelectedAccountKey();
  const { data: marginfiAccounts, refetch: refetchMarginfiAccounts } = useMarginfiAccountAddresses();

  const { user, balances, getMixinClient, setKeystore, clear, register, connected } = useComputerStore((s) => ({
    user: s.user,
    balances: s.balances,
  ...
  const handleConnected = () => {
    // console.log("🎉 Navbar: Mixin 连接成功，刷新用户数据");
    // 立即刷新用户数据，让 FluxlendProvider 能够感知到状态变化
    // 账户设置将由上面的 useEffect 自动处理
    // refreshUserData();
  };
  ...
  const handleLogout = () => {
    ...
    // 清空 marginfi 账户选择
    localStorage.removeItem("marginfi-selected-account-key");
      ...
      <nav className="fixed w-full top-0 h-[64px] z-50 bg-background">
        <div className="w-full flex justify-between items-center h-16 text-sm font-[500] text-[#868E95] z-10 border-b-[0.5px] border-[#1C2125] px-4">
          <div className="h-full w-1/2 z-10 flex items-center gap-8">
            ...
            <LoginModal open={showLoginModal} onClose={() => setShowLoginModal(false)} onConnected={handleConnected} />
