...
async function fetchStateMetaData(marginfiConfig: MarginfiConfig) {
  ...
  if (marginfiConfig.environment === "production") {
    ...
    // ]);
    const bankMetadataJson = (await import("../../../store/mainnet-domelend-meta.json")) as {
  ...
  // );
  const stakedAssetBankMetadataMap = new Map<string, BankMetadata>();
  const stakedAssetTokenMetadataMap = new Map<string, TokenMetadata>();
